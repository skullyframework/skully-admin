...
(function() {
	var template = '<span id="{id}"' +
		...
		' role="presentation">' +
			...
			// jay: added input with events.
			'<input onkeyup="return CKEDITOR.tools.callFunction({keyupInputFn}, event)" onclick="return CKEDITOR.tools.callFunction({clickInputFn}, event)" id="{id}_text" class="cke_combo_text cke_combo_inlinelabel" value="{label}"/>' +
		...
		proto: {
			...
			render: function( editor, output ) {
				...
				function updateState() {
					...
					// jay: text in combo boxes must show past value.
//					this.setValue( '' );
				}
				...
				// jay: Key event functions
				var me = this;
				var keyupInputFn = CKEDITOR.tools.addFunction( function( ev ) {
					if ( me.onKeyup ) {
						if (typeof(me.keyupTime) != 'undefined') {
							clearTimeout(me.keyupTime);
						}
						// Key up must be given time to wait for user to finish typing.
						me.keyupTime = setTimeout(function() {
							me.onKeyup.call( me, ev.srcElement.value, true );
							ev.srcElement.focus();
						}, 800);
					}
				}, this );

				var clickInputFn = CKEDITOR.tools.addFunction( function( ev ) {
					ev.srcElement.focus();
				}, this );
				...
				var params = {
					...
					focusFn: focusFn,
					clickFn: clickFn,
					// jay: attaching the events
					clickInputFn: clickInputFn,
					keyupInputFn: keyupInputFn
				};
			...
			setValue: function( value, text ) {
				...
				if ( textElement ) {
					...
					// jay: Change from setText to setValue, since new elements are input boxes.
					textElement.setValue( typeof text != 'undefined' ? text : value );
