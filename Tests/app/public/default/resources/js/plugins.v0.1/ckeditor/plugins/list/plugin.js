...
(function() {
	...
	function createList( editor, groupObj, listsCreated ) {
		...
		var insertAnchor = listContents[ listContents.length - 1 ].getNext(),
			...
			// jay: set style
//		listNode.setStyles({'margin': 0, 'padding-left': 'auto'});
		if (this.type == 'ol') {
			listNode.setStyles({'padding-left': '40px', 'margin-top': '16px', 'margin-left': '0', 'margin-bottom': '16px'});
		}
		else {
			listNode.setStyles({'padding-left': '40px', 'margin-top': '0', 'margin-left': '0', 'margin-bottom': '16px'});
		}
		...
		while ( listContents.length ) {
			...
			if ( contentBlock.is( 'pre' ) || headerTagRegex.test( contentBlock.getName() ) )
				...
				// jay: set style
				listItem.setStyles({'margin-top': 0, 'margin-right': 0, 'margin-bottom': '3px', 'margin-left': 0});
	...
	// jay: Add margin and padding to list html.
	function setupListHtml(editor) {
		var data = editor.getData();
		if (data.indexOf('<ul>') !== -1 || data.indexOf('<ol>') !== -1) {
//			var newData = data.replace(/\<ul\>/g, '<ul style="margin: 0; padding-left: auto;">').replace(/\<ol\>/g, '<ol style="margin: 0; padding-left: auto;">').replace(/\<li\>/g, '<li style="margin: 0">');
//			var newData = data.replace(/\<ul\>/g, '<ul style="padding-left: auto;">').replace(/\<ol\>/g, '<ol style="padding-left: auto;">').replace(/\<li\>/g, '<li style="margin:0;">');
			var newData = data.replace(/\<ul\>/g, '<ul style="padding-left: 40px; margin-top: 0; margin-left: 0; margin-bottom: 16px;">').replace(/\<ol\>/g, '<ol style="padding-left: 40px; margin-top: 16px; margin-left: 0; margin-bottom: 16px;">').replace(/\<li\>/g, '<li style="margin: 0 0 3px 0;">');
			editor.editable().setHtml(newData);
		}
	}
