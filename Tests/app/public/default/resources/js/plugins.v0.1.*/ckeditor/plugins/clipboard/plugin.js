		...
		init: function( editor ) {
			...
			editor.on( 'paste', function( evt ) {
				...
				if ( type == 'auto' ){
					...
					//call the htmlTextification or htmlifiedTextHtmlification based on true type
					//we need to unify text markup, but 'auto' would skipped the process before
					if ( trueType == 'htmlifiedtext' )
						data = htmlifiedTextHtmlification( editor.config, data );
					// Strip presentional markup & unify text markup.
					else if ( trueType == 'html' ) {
						// Init filter only if needed and cache it.
						data = htmlTextification( editor.config, data, textificationFilter || ( textificationFilter = getTextificationFilter( editor ) ) );
					}
				}
	...
	function initClipboard( editor ) {
		...
		function getClipboardDataByPastebin( evt, callback ) {
			...
			setTimeout( function() {
				...
				pastebin.$.getChildren().each(function(el) {
...
//			data = data.replace( /<(\/)?p>/g, '<$1div>' );
			// regex is not too accurate, it won't work for <p class="">
			//change the regex to the following code
			data = data.replace( /<(\/)?p( |>)?/g, '<$1div$2');
		}
